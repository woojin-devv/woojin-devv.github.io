---
title: "3주차 - 네트워크"
description: "참고자료 HTTP 1 Vs HTTP 2 Vs HTTP 3! HTTP 헤더(Header) 사용자가 HTTP 요청을 하게 되면 헤더와 바디를 주고 받는다. 헤더: 바디를 설명하는 정보를 포함한 정보 묶음 헤더는 콜론(:)으로 구분되는 key value 형태로 설정됨 HTTP 요청을 할 때 "
date: 2026-04-02
...
1. **키 교환 (Key Exchange)**
    - 클라이언트와 서버가 만난 적 없지만, 똑같은 대칭키를 나누어 가져야할 때 사용
        - **ECDHE(Elliptic Curve Diffie–Hellman Ephemeral)**
2. **전자 서명 (Digital Signature)**
    - 해당 서버가 진짜가 맞음 (인증서 위조되지 않음)을 증명할 때사용
        - **ECDSA**

![alt text](/assets/img/cs-interview-3/12.png)

### 1. ClientHello

클라이언트는 서버에게 먼저 ClientHello 메시지를 보낸다. 

- 클라이언트가 지원하는 TLS 버전 목록
- 클라이언트가 지원하는 Cipher Suites 목록
    
    클라이언트 랜덤값 (난수로 세션 키 생성에 사용됨)
    
- 클라이언트 측 ECDHE 공개키

### 2. ServerHello

서버는 클라이언트의 메시지를 분석한 후 다음과 같은 과정을 진행

- 클라이언트와 공통으로 지원하는 TLS 버전 중 가장 높은 버전 선택
- 클라이언트가 제안한 Cipher Suites 중 서버가 지원하는 Cipher Suites 선택
- ServerHello 메시지를 보내면서 다음을 포함
    - 선택된 TLS 버전
    - 선택된 Cipher Suites
    - 서버 랜덤값
    - 서버측 ECDHE 공개키
...
- Encrypted Extensions: SNI, ALPN
- Certificate
- CertificateVertify

### 4. Finished

클라이언트와 서버는 각각 Finished 메시지를 교환합니다. 이후의 통신은 세션키로 암호화되며, 이후 모든 통신은 AES 같은 대칭 암호화 방식으로 보호

### ECDHE – 타원곡선 암호화

ECDHE에서 먼저 DH는 Diffie-Hellman을 의미

- Diffie-Hellman 알고리즘은 서로 공개값 공유, 비밀값과 혼합, 혼합값과 공유, 각자의 비밀값과 혼합해서 공통의 암호키를 만드는 알고리즘
...
- 해당 식으로부터 $A, g, p$를 알아도 $a$를 계산하는 것 힘듦
    - 예)
        
        $g=5, p=23, A= 8$이라고 할 때,
        
        $a$를 찾기 위해서는 brute force로 전부 연산해야함.
        

### TLS에서 ECC를 사용하는 이유 (RSA와 비교)

| **비교 항목** | **RSA (과거 표준)** | **ECC (현대 표준)** |
| --- | --- | --- |
| **보안 원리** | 큰 수의 소인수분해 | **타원곡선 이산로그 문제** |
| **키 사이즈** | **3072 bit** (매우 큼) | **256 bit** (매우 작음) |
| **보안 강도** | 보통 | **매우 높음** |
| **연산 속도** | 무거움 (배터리 소모↑) | **가볍고 빠름 (모바일 최적화)** |